...
build_basemap <- function(
    ...
    crs = 27700,
    aspect_ratio = 1,
    expand_dir = c("eastwards", "northwards"),
    cache_tiles = FALSE,
    allow_premium = FALSE,
    chatty = TRUE) {
  ...
  if (crs == 27700) {
    raster_data <- generate_png_27700(bbox = bbox, zoom = zoom, type = type, aspect = aspect_ratio, allow_premium = allow_premium, chatty = chatty)
  }

  if (crs == 3857) {
    raster_data <- generate_png_3857(bbox = bbox, zoom = zoom, type = type, aspect = aspect_ratio, allow_premium = allow_premium, chatty = chatty)
  }
  ...
  if (!cache_tiles) {
    ...
    do.call(terra::merge, raster_list)
